...
namespace ProgrammingAssignments
{
    class ArrayProblems
    {
        public static int evenOddSumByRemoveItem(List<int> A)
        {
            int N = A.Count;
            int ans = 0;
            var PreEven = A.ConvertAll<int>(c => c);
            var PreOdd = A.ConvertAll<int>(c => c);
            var PostEven = A.ConvertAll<int>(c => c);
            var PostOdd = A.ConvertAll<int>(c => c);
        ...
        private static void Oddeven()
        {
            ...
            for (int i = 0; i < T; i++)
            {
                ...
                int[] A = Array.ConvertAll(Console.ReadLine().Trim().Split(' '), Int32.Parse);
